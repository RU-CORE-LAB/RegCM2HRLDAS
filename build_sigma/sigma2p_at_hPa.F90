...
program sigma2p
  ...
  integer(ik4) , parameter :: np = 13
  real(rk4) , dimension(np) :: plevs

  character(256) :: prgname , ncsfile , ncpfile, pre_ncpfile, bname   ! NICK
  ...
  data plevs /1010.8487,1000.,925.,850.,700.,600.,500.,400.,300.,250.,200.,150.,100./
...
!$OMP THREADPRIVATE(xvar,pvar)
  ...
  !:::::::::::::::::::::::::::::::::::::::::::::::::::::::
  !
  ! Add get arg ouput path.
  !
  ! get arguments
  
  call get_command_argument(1, ncsfile)
  call get_command_argument(2, pre_ncpfile)

  ! find last '/' and last '.'
  iid1 = index(ncsfile, '/', back=.true.)
  iid2 = index(ncsfile, '.', back=.true.)

  if (iid2 == 0 .or. iid2 <= iid1) then
     print *, 'ERROR: invalid input filename:', trim(ncsfile)
     stop 1
  endif

  ! extract basename without extension
  bname = ncsfile(iid1+1:iid2-1)

  ! make sure output directory exists
  call execute_command_line('mkdir -p ' // trim(pre_ncpfile))

  ! build output filename
  ncpfile = trim(pre_ncpfile)//'/'//trim(bname)//'_pressure.nc'

  !print *, 'Input : ', trim(ncsfile)
  !print *, 'Output: ', trim(ncpfile)
